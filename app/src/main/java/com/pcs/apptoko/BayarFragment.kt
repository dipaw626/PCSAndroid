...
class BayarFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        btnSimpanBayar.setOnClickListener{
            api.postTransaksi(token.toString(),
            adminId.toString().toInt(),total.toString().toInt()).enqueue(object : retrofit2.Callback<TransaksiResponsePost> {
                override fun onResponse(
                    ...
                    for (item in my_cart!!) {
                        api.postItemTransaksi(token.toString(),id_transaksi.toString().toInt(),item.id, item.qty, item.harga).
                                enqueue(object : retrofit2.Callback<itemTransaksiResponsePost> {
                                    override fun onResponse(
